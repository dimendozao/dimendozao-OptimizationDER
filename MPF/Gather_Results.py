# -*- coding: utf-8 -*-
"""
Created on Fri Apr 19 12:03:01 2024
...
for ct in range(nci):
    matsol=[['']*nmatsol for i in range(nca)]
    for ca in range(nca):
        ...
        for i in range(nform):
            if i<nform-1 and not i==6 and not i==11:
                ...
                datadict.update({form[i]:pd.read_csv('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city[ct]+'\\'+case[ca]+'\\'+conv[i]+'\\Results.csv')})
        ...
        matsol[ca][0]='$'+np.format_float_scientific(np.max(v[-1]), precision=2)+'$'
        matsol[ca][1]='$'+np.format_float_scientific(np.min(v[-1]), precision=2)+'$'
        matsol[ca][2]='$'+np.format_float_scientific(np.max(np.abs(ph[-1])), precision=2)+'$'
        matsol[ca][3]='$'+np.format_float_scientific(np.min(np.abs(ph[-1])), precision=2)+'$'
        matsol[ca][4]='$'+np.format_float_scientific(np.sum(datadict[form[-1]]['pg'].to_numpy()), precision=2)+'$'
        matsol[ca][5]='$'+np.format_float_scientific(np.sum(datadict[form[-1]]['qg'].to_numpy()), precision=2)+'$'
        matsol[ca][6]='$'+np.format_float_scientific(np.sum(datadict[form[-1]]['pl'].to_numpy()), precision=2)+'$'
        matsol[ca][7]='$'+np.format_float_scientific(np.sum(datadict[form[-1]]['ql'].to_numpy()), precision=2)+'$'
