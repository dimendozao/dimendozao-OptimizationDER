# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 15:31:58 2024
...
zb=  m.addMVar(num_nodes,vtype=gp.GRB.BINARY,name='zb')
cb= m.addMVar(num_nodes,lb=0, ub=cbm, name='cb')
pih= m.addMVar((H,num_nodes),lb=-cbm,ub=cbm,name='pih')
pbh= m.addMVar((H,num_nodes),lb=-cbm,ub=cbm,name='pbh')
cap= m.addMVar((H,num_nodes),lb=0,ub=cbm,name='cap')
capz= m.addMVar(num_nodes,lb=0,ub=cbm, name='cb')
...
m.addConstrs((EqNp[h][i]==pgref[h][i]+pbh[h][i]+(ppv[i]*radmean[h,0])+pgen[i]-(pdm[i]*clusters[1][h]) for h in range(H) for i in range(num_nodes)), name='c-pf-p')
...
m.addConstr(gp.quicksum(z[i] for i in range(num_nodes))==npv, name='c-zpv')

m.addConstrs((pbh[h][i]<=zb[i]*cbm for h in range(H) for i in range(num_nodes)), name='c-bat1')
m.addConstrs((pbh[h][i]>=zb[i]*-cbm for h in range(H) for i in range(num_nodes)), name='c-bat2')
m.addConstrs((pbh[h][i]>=pih[h][i]-cbm*(1-zb[i]) for h in range(H) for i in range(num_nodes)), name='c-bat3')
m.addConstrs((pbh[h][i]<=pih[h][i]+cbm*(1-zb[i]) for h in range(H) for i in range(num_nodes)), name='c-bat4')
m.addConstr(gp.quicksum(zb[i] for i in range(num_nodes))==nb, name='c-zb')

m.addConstrs((pih[h][i]>=-cb[i] for h in range(H) for i in range(num_nodes)), name='c-bmin')
m.addConstrs((pih[h][i]<=cb[i] for h in range(H) for i in range(num_nodes)), name='c-bmax')

m.addConstrs((cap[h][i]<=cb[i] for h in range(H) for i in range(num_nodes)), name='c-cmax')
m.addConstrs((capz[i]<=cb[i] for i in range(num_nodes)), name='c-czmax')

m.addConstrs((cap[h][i]==capz[i]+pbh[h][i] for h in range(H) for i in range(num_nodes) if h==0), name='c-capz')
m.addConstrs((cap[h][i]==cap[h-1][i]+pbh[h][i] for h in range(H) for i in range(num_nodes) if h>0), name='c-cap')

m.addConstrs((cap[23][i]==capz[i] for i in range(num_nodes)), name='c-cap023')
...
zbo= np.array(values[((3*num_nodes*H)+(3*num_lines*H))+(3*num_nodes):((3*num_nodes*H)+(3*num_lines*H))+(4*num_nodes)]) 
cbo= np.array(values[((3*num_nodes*H)+(3*num_lines*H))+(4*num_nodes):((3*num_nodes*H)+(3*num_lines*H))+(5*num_nodes)]) 
pihv= np.array(values[((3*num_nodes*H)+(3*num_lines*H))+(5*num_nodes):((4*num_nodes*H)+(3*num_lines*H))+(5*num_nodes)]) 
pbhv= np.array(values[((4*num_nodes*H)+(3*num_lines*H))+(5*num_nodes):((5*num_nodes*H)+(3*num_lines*H))+(5*num_nodes)])
capv= np.array(values[((5*num_nodes*H)+(3*num_lines*H))+(5*num_nodes):((6*num_nodes*H)+(3*num_lines*H))+(5*num_nodes)])
capzo= np.array(values[((6*num_nodes*H)+(3*num_lines*H))+(5*num_nodes):((6*num_nodes*H)+(3*num_lines*H))+(6*num_nodes)])
...
piho=np.zeros([H,num_nodes])
pbho=np.zeros([H,num_nodes])
capbo=np.zeros([H,num_nodes])
...
plt.plot(np.sum(pbho,axis=1))
plt.legend(['pg','pv','pd','pb'])
