...
for h in range(H):
    cn[h] = cvx.Variable((num_nodes,num_nodes))
    sn[h] = cvx.Variable((num_nodes,num_nodes))
...
for h in range(H):
    ...
    res+=   [sn[h]<=snmax[h]]
    res+=   [sn[h]>=snmin[h]]
    res+=   [pref[h]+pgen-pdm[h]==cvx.diag(ymr@cn[h])+cvx.diag(ymi@sn[h])]        
    res+=   [qref[h]+qgen-qdm[h]==cvx.diag(ymr@sn[h])-cvx.diag(ymi@cn[h])] 
...
for h in range(H):
    pl[h]=cvx.sum(cvx.diag(ymr@cn[h])+cvx.diag(ymi@sn[h]))
    ql[h]=cvx.sum(cvx.diag(ymr@sn[h])-cvx.diag(ymi@cn[h])) 
