# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 17:19:42 2024
...
ud= m.addMVar((H,num_nodes,ndiv),lb=0,ub=1.2,name='udiv')
wr= m.addMVar((H,num_lines),lb=wrmin,ub=wrmax,name='wr')
wrd= m.addMVar((H,num_lines,ndiv),lb=0,ub=1.2,name='wrdiv')
wi= m.addMVar((H,num_lines),lb=wimin,ub=wimax,name='wi')
wid= m.addMVar((H,num_lines,ndiv),lb=-1.2,ub=1.2,name='wrdiv')
    ...
    for i in range(num_nodes):        
        for k in range(num_lines):
            if i==(branch['i'][k]-1):
                ...
                yr=np.real(y)
                yi=np.imag(y)
                EqNp[h][i]+=sum(ud[h][i]*(yr/ndiv))-sum(wrd[h][k]*(yr/ndiv))-sum(wid[h][k]*(yi/ndiv))
                EqNq[h][i]+=sum(-ud[h][i]*(yi/ndiv))+sum(wrd[h][k]*(yi/ndiv))-sum(wid[h][k]*(yr/ndiv))
            if i==(branch['j'][k]-1):
                y  = 1/(branch['r'][k] + 1j*branch['x'][k])
                yr=np.real(y)
                yi=np.imag(y)
...
m.addConstrs((sum(ud[h][i])==u[h][i] for h in range(H) for i in range(num_nodes)),name='c-udiv')
m.addConstrs((sum(wrd[h][i])==wr[h][i] for h in range(H) for i in range(num_lines)),name='c-wrdiv')
m.addConstrs((sum(wid[h][i])==wi[h][i] for h in range(H) for i in range(num_lines)),name='c-wrdiv')
