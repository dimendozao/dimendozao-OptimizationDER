# -*- coding: utf-8 -*-
"""
Created on Tue Apr  2 08:14:07 2024
...
wrmax=np.multiply(wrmax,idx)
wrmin=np.multiply(wrmin,idx)

wimax=np.multiply(wimax,idx)
wimin=np.multiply(wimin,idx)         
...
res+=   [sref+sgen-sdm==cvx.diag(w@cvx.conj(ym))]        
...
pl=cvx.real(cvx.trace(w@cvx.conj(ym)))
ql=cvx.imag(cvx.trace(w@cvx.conj(ym)))
res +=[pl>=0]
res +=[ql>=0]    
...
for h in range(H):
    sdm.value=sd[h]
    OPFSOC.solve(solver=cvx.MOSEK,canon_backend=cvx.SCIPY_CANON_BACKEND,enforce_dpp=True,accept_unknown=True,mosek_params = {'MSK_IPAR_INTPNT_SOLVE_FORM':'MSK_SOLVE_DUAL','MSK_IPAR_PRESOLVE_USE':'MSK_PRESOLVE_MODE_ON'},verbose=True)    
    "----- Print results -----"
