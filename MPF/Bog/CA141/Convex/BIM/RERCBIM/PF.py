...
for k in range(num_lines):
    ...
    yk[k]=1/(branch['r'][k] + 1j*branch['x'][k]) 
...
for h in range(H):
    for i in range(num_nodes):        
        for k in range(num_lines):
            if i==fr[k]:                
                EqN[h][i]+=(u[h][i]-w[h][k])*yk[k].conjugate()
            if i==to[k]:               
                EqN[h][i]+=(u[h][i]-cvx.conj(w[h][k]))*yk[k].conjugate()    

for h in range(H):
    for i in range(num_nodes):
        res +=  [EqN[h][i]==sref[h][i]+sgen[i]-sd[h][i]] 
...
obj = cvx.Minimize(cvx.sum(cvx.real(sl))+cvx.sum(cvx.imag(sl)))
#obj = cvx.Minimize(cvx.sum(pl)+cvx.sum(ql))
#obj = cvx.Minimize(1)

"-------Problem/solver Setup--------"
OPFSOC = cvx.Problem(obj,res)
#OPFSOC.solve(solver=cvx.MOSEK,verbose=True)
#OPFSOC.solve(solver=cvx.MOSEK,accept_unknown=True,mosek_params = {'MSK_IPAR_INTPNT_SOLVE_FORM':'MSK_SOLVE_DUAL','MSK_IPAR_PRESOLVE_USE':'MSK_PRESOLVE_MODE_OFF'},verbose=True)    
...
for h in range(H):
    for i in range(num_nodes):
        for k in range(num_lines):
            if i==fr[k]:                
                Equ[h][i]+=(uo[h][i]-wo[h][k])*yk[k].conjugate()
            if i==to[k]:                
                Equ[h][i]+=(uo[h][i]-wo[h][k].conjugate())*yk[k].conjugate()    
...
for h in range(H):
    ...
    for k in range(num_lines):
        ph[h][to[k]]=ph[h][fr[k]]-np.angle(wo[h][k])
