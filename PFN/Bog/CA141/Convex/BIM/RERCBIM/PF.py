...
sgen=np.zeros(num_nodes,dtype='complex')
...
if ngen>0:
    for i in range(ngen):
        sgen[bus['i'][i]-1]=gen['pi'][i]+1j*gen['pi'][i]
...
for i in range(num_nodes):        
    for k in range(num_lines):
        if i==fr[k]:            
            EqN[i]+=(u[i]-w[k])*cvx.conj(yl[k])
        if i==to[k]:
            EqN[i]+=(u[i]-cvx.conj(w[k]))*cvx.conj(yl[k])    
...
pl=cvx.sum(cvx.real(cvx.hstack(EqN)))
ql=cvx.sum(cvx.imag(cvx.hstack(EqN)))
...
# pinfeas2=pg-np.real(equ2)-np.real(sd)
# qinfeas2=qg-np.imag(equ2)-np.imag(sd)
