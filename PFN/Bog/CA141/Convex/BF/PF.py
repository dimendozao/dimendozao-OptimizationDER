...
qik= m.addMVar(num_lines,lb=-GRB.INFINITY,ub=GRB.INFINITY,name='qik')
pk= m.addMVar(num_lines,lb=-GRB.INFINITY,ub=GRB.INFINITY,name='pk')
qk= m.addMVar(num_lines,lb=-GRB.INFINITY,ub=GRB.INFINITY,name='qk')
...
m.addConstrs((qik[k]*qvn[fr[k]]>=pk[k]*pk[k]+qk[k]*qk[k] for k in range(num_lines)),name='c-socp')
...
qvno=np.array(values[2*num_nodes:3*num_nodes])
qiko=np.array(values[3*num_nodes:(3*num_nodes)+(num_lines)])
pko=np.array(values[(3*num_nodes)+(num_lines):(3*num_nodes)+(2*num_lines)])
qko=np.array(values[(3*num_nodes)+(2*num_lines):(3*num_nodes)+(3*num_lines)])
...
# vrec=(vo*np.cos(pho)+1j*vo*np.sin(pho))
# equ=np.array(Equp)+1j*np.array(Equq)
# sg=pgo+1j*qgo
...
# sinfeas1=sg-equ-sd
# pinfeas1=np.real(sinfeas1)
# qinfeas1=np.real(sinfeas1)
...
# sinfeas2= sg-equ2-sd    
# pinfeas2=np.real(sinfeas2)
# qinfeas2=np.real(sinfeas2)


#output=np.vstack((vo,pho,Equp,Equq,ploss,qloss,pgo,qgo,pinfeas1,pinfeas2,qinfeas1,qinfeas2,t)).T
