...
ampl.eval(
    ...
    minimize Losses:
         sum{i in N, j in N} cn[i,j]*yr[i,j]
       + sum{i in N, j in N} sn[i,j]*yi[i,j] 
       + sum{i in N, j in N} sn[i,j]*yr[i,j]
       - sum{i in N, j in N} cn[i,j]*yi[i,j];
    ...
    subject to PBP:
        sum {i in N,j in N} cn[i,j]*yr[i,j] + sum {i in N,j in N} sn[i,j]*yi[i,j]>=0;
    subject to QBP:
        sum {i in N,j in N} sn[i,j]*yr[i,j] - sum {i in N, j in N} cn[i,j]*yi[i,j]>=0;
