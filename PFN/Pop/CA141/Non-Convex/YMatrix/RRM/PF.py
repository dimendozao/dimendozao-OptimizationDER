...
ampl.eval(
    ...
    minimize Losses:
         sum{i in N, j in N: idx[i,j]=1} cn[i,j]*yr[i,j]
       + sum{i in N, j in N: idx[i,j]=1} sn[i,j]*yi[i,j] 
       + sum{i in N, j in N: idx[i,j]=1} sn[i,j]*yr[i,j]
       - sum{i in N, j in N: idx[i,j]=1} cn[i,j]*yi[i,j];
    
    subject to PB {i in N}: 
        pg[i]-pd[i] = sum {j in N: idx[i,j]=1} cn[i,j]*yr[i,j] + sum {j in N: idx[i,j]=1} sn[i,j]*yi[i,j];  
        
    subject to QB {i in N}: 
        qg[i]-qd[i] = sum {j in N: idx[i,j]=1} sn[i,j]*yr[i,j] - sum {j in N: idx[i,j]=1} cn[i,j]*yi[i,j];    
    
    subject to SOC {i in N, j in N: idx[i,j]=1 and j>i}:
        (cn[i,j]*cn[i,j])+(sn[i,j]*sn[i,j])=cn[i,i]*cn[j,j];
        
    subject to Hermit1 {i in N, j in N: idx[i,j]=1 and  j>i}:
        cn[i,j]=cn[j,i];
        
    subject to Hermit2 {i in N, j in N: idx[i,j]=1 and j>i}:
        sn[i,j]=-sn[j,i];     
