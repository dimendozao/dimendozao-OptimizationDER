...
svect=np.zeros(num_nodes)
svect[iref]=1

"----- Optimization model -----"

sref = cvx.Variable(complex=True,name='sref')
...
res+=[(sref*svect)+sgen-sd==cvx.diag(w@ym.conjugate())]        


res +=  [cvx.real(sref)>=prefmin[iref]]
res +=  [cvx.real(sref)<=prefmax[iref]]
res +=  [cvx.imag(sref)>=qrefmin[iref]]
res +=  [cvx.imag(sref)<=qrefmax[iref]]
...
pl=cvx.real(cvx.trace(w@ym.conjugate()))
ql=cvx.imag(cvx.trace(w@ym.conjugate())) 


res +=  [pl>=0]
res +=  [ql>=0]
res +=  [cvx.real(cvx.sum((sref*svect)-sd))>=0]
res +=  [cvx.imag(cvx.sum((sref*svect)-sd))>=0]
...
OPFSOC.solve(solver=cvx.MOSEK,accept_unknown=True,mosek_params = {'MSK_IPAR_INTPNT_SOLVE_FORM':'MSK_SOLVE_DUAL','MSK_IPAR_PRESOLVE_USE':'MSK_PRESOLVE_MODE_ON','MSK_DPAR_PRESOLVE_TOL_X':1e-10},verbose=True)    
...
pg=np.real(sref.value)*svect
qg=np.imag(sref.value)*svect
...
vrec=(v*np.cos(ph)+1j*v*np.sin(ph))
...
pinfeas1=pg-Equp-np.real(sd)
qinfeas1=qg-Equq-np.imag(sd)

equ2= np.multiply(vrec,np.matmul(ym.conjugate(),vrec.conjugate()))   
pinfeas2=(pg*svect)-np.real(equ2)-np.real(sd)
qinfeas2=(qg*svect)-np.imag(equ2)-np.imag(sd)
   
output=np.vstack((v,ph,Equp,Equq,ploss,qloss,pg,qg,pinfeas1,pinfeas2,qinfeas1,qinfeas2,t)).T
