# -*- coding: utf-8 -*-
"""
Created on Mon May  6 09:22:35 2024
...
m.addConstrs(((cn[h]@ymr+sn[h]@ymi)[i][i]==pgref[h][i]+(ppv[i]*imeans[h]*pveff)-pdh[h][i] for h in range(H) for i in range(num_nodes)), name='c-pf-p')
m.addConstrs(((sn[h]@ymr-cn[h]@ymi)[i][i]==qgref[h][i]-qdh[h][i] for h in range(H) for i in range(num_nodes)), name='c-pf-q')
m.addConstrs((cn[h][i][i]*cn[h][j][j]>=cn[h][i][j]*cn[h][i][j]+sn[h][i][j]*sn[h][i][j] for h in range(H) for i in range(num_nodes) for j in range(num_nodes) if idx2[i][j]==1 and i<j),name='c-socp')
...
m.addConstrs((gp.quicksum((cn[h]@ymr+sn[h]@ymi)[i][i] for i in range(num_nodes))==pl[h] for h in range(H)), name='c-ppl')
m.addConstrs((gp.quicksum((sn[h]@ymr-cn[h]@ymi)[i][i] for i in range(num_nodes))==ql[h] for h in range(H)), name='c-pql')
