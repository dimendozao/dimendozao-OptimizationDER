# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 08:19:45 2024
...
ampl.get_parameter("pvcmax").set_values([pvcmax])
ampl.get_parameter("npv").set_values([npv])               
...
ampl.eval(
    ...
    subject to PB {i in N}: 
        pg[i]+ppv[i]-pd[i] = sum {k in L: fr[k]=i} cii[i]*yr[k]
    ...
    subject to PVL1 {i in N}:
        ppv[i]<=pvcmax*zpv[i];
    
    subject to PVL2 {i in N}:
        ppv[i]<=pv[i];
        
    subject to PVL3 {i in N}:
