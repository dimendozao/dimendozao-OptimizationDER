...
for h in range(H):
    ...
    res+= [pref[h]+(ppv[i]*imeans[h]*pveff)+pgen-pdh[h]==cvx.diag(cn[h]@ymr)+cvx.diag(sn[h]@ymi)]        
...
OPFSOC.solve(solver=cvx.MOSEK,mosek_params = {'MSK_IPAR_INTPNT_SOLVE_FORM':'MSK_SOLVE_PRIMAL','MSK_IPAR_PRESOLVE_USE':'MSK_PRESOLVE_MODE_OFF','MSK_IPAR_INFEAS_REPORT_AUTO':'MSK_ON'},verbose=True)    
