# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 08:14:51 2024
...
ampl.eval(
    r"""
    var  cn{h in H,i in N, j in N: idx1[i,j]=1} >= cnmin[i,j], <= cnmax[i,j];
    var  sn{h in H,i in N, j in N: idx1[i,j]=1} >= snmin[i,j], <= snmax[i,j];
    ...
    minimize Losses:
         sum{h in H} pl[h] +sum{h in H} ql[h];
    ...
    subject to SOC {h in H,i in N, j in N: idx2[i,j]=1}:
        cn[h,i,i]*cn[h,j,j]>=cn[h,i,j]^2+sn[h,i,j]^2;
        
    subject to Hermit1 {h in H,i in N, j in N: idx2[i,j]=1}:
        cn[h,i,j]=cn[h,j,i];
        
    subject to Hermit2 {h in H,i in N, j in N: idx2[i,j]=1}:
        sn[h,i,j]=-sn[h,j,i];
    ...
    subject to PL{h in H}:
        pl[h]=sum {i in N, j in N: idx1[i,j]=1} cn[h,i,j]*yr[i,j] + sum {i in N,j in N: idx1[i,j]=1} sn[h,i,j]*yi[i,j];    
    subject to QL{h in H}:
        ql[h]=sum {i in N, j in N: idx1[i,j]=1} sn[h,i,j]*yr[i,j] - sum {i in N,j in N: idx1[i,j]=1} cn[h,i,j]*yi[i,j]; 
...
#ampl.solve(solver="highs", highs_options='outlev=1 miploglev=2 socp=2')

ampl.option['email']='dimendozao@unal.edu.co'
ampl.solve(solver="kestrel",kestrel_options="solver=mosek",mosek_options='outlev=1')
