# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 19:29:11 2024
...
dsocx=np.linspace(-1,1,npwl)
pbcx=np.linspace(0,pbcmax,npwl)

pbx=np.zeros([npwl,npwl])

for i in range(npwl):
    for j in range(npwl):
        pbx[i][j]=dsocx[i]*pbcx[j]
...
dsoc= m.addMVar(H,lb=-1,ub=1,name='dsoc')
...
m.addConstrs((EqNp[h][i]==pgref[h][i]+(ppv[i]*imeans[h]*pveff)+(pb[h]*zb[h][i])+pgen[i]-(pdm[i]*dmeans[h][cnode[i]]) for h in range(H) for i in range(num_nodes)), name='c-pf-p')
m.addConstrs((EqNq[h][i]==qgref[h][i]+qgen[i]-(qdm[i]*dmeans[h][cnode[i]]) for h in range(H) for i in range(num_nodes)), name='c-pf-q')

m.addConstrs((gp.quicksum(zb[h][i] for i in range(num_nodes))==1 for h in range(H)),name='c-pb5')


m.addConstr(dsoc[0]==soc0-soc[0], name='c-soc0')
m.addConstrs((dsoc[h]==soc[h-1]-soc[h] for h in range(H) if h>0), name='c-soc')
m.addConstr(soc[H-1]==soc0, name='c-soc024')
m.addConstrs((pb[h]==dsoc[h]*pbc for h in range(H)),name='c-socpb')
