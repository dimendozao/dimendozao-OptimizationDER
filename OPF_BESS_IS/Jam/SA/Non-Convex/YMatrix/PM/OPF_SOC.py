...
ampl.eval(
    ...
    var pbz{h in H, i in N} >=-pbcmax, <=pbcmax;
    var soc{h in H} >=0, <=1;
    var soc0 >=0,<=1;
    var dsoc{h in H} >=-1,<=1;
...
gap[0]=100*(np.max([gapn1,gapn2])-np.min([gapn1,gapn2]))/np.max([gapn1,gapn2])
...
out1=np.vstack((ploss,qloss,pgo[:,iref],qgo[:,iref],t,gap,imeans,pbcout,pbo,soc0out,soco)).T
