    ...
    for ca in range(nca):
        ...
        for i in range(nform):
            if i==0:
                ...
                cap0=datadict[form[i]]['cap0'].to_numpy()/datadict[form[i]]['pbc'][0]
                datadict[form[i]]['cap0']=cap0
                soc=datadict[form[i]]['caph'].to_numpy()/datadict[form[i]]['pbc'][0]
                datadict[form[i]]['caph']=soc
        ...
        zbcols=cols.drop(cols[:4*num_nodes+11])
        zbcols=zbcols.drop(cols[5*num_nodes+11:])
        zbcols=zbcols.to_list()
        dccols=cols.drop(cols[:5*num_nodes+11])
        dccols=dccols.drop(cols[5*num_nodes+16:])
        dccols=dccols.to_list()
        ppvcols=cols.drop(cols[:5*num_nodes+16])
        ppvcols=ppvcols.drop(cols[6*num_nodes+16:])
        ...
        for i in range(nform):
            ...
            for h in range(H):
                ...
                for j in range(num_nodes):
                    ...
                    pdmc[j]=pdm[j]*dc[h][cnode[j]]
                    qdmc[j]=qdm[j]*dc[h][cnode[j]]
                    if datadict[form[i]][zbcols[j]][h]==1:
                        pbmat[j]=datadict[form[i]]['pb'][h]                    
                
                infeas1[i]+=np.abs(np.sum(pgmat+(ppvmat*ic[h]*pveff)+pbmat-eqpmat-pdmc))+np.abs(np.sum(qgmat-eqqmat-qdmc))                       
                infeas2[i]+=np.abs(np.sum(pgmat+(ppvmat*ic[h]*pveff)+pbmat-np.real(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-pdmc))+np.abs(np.sum(qgmat-np.imag(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-qdmc))
        ...
        for h in range(H):
            if datadict[form[idxd[0]]]['pb'][h]>=0:
                pbdhca[ct][ca]+=datadict[form[idxd[0]]]['pb'][h]
            else:
                pbchca[ct][ca]+=datadict[form[idxd[0]]]['pb'][h]            
            for i in range(num_nodes):
                plca[ct][ca]+=pdm[i]*dc[h][cnode[i]]
                pvca[ct][ca]+=ppv[i]*ic[h]*pveff
        ...
        pbca[:,ct,ca]=datadict[form[idxd[0]]]['pb'].to_numpy()
