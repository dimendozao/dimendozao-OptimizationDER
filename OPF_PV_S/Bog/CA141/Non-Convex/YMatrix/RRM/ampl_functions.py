# -*- coding: utf-8 -*-
"""
Created on Thu May  9 15:23:59 2024
...
ampl.eval(
    ...
    function gsl_ran_logistic_P;
    function gsl_cdf_lognormal_P;
    function gsl_cdf_gaussian_P;
    function gsl_ran_rayleigh_P;
    function gsl_cdf_weibull_P;
        
    subject to Probi1{h in H: besti[h]=1}:
        probi[h]=gsl_cdf_exponential_P(ic[h],iparams[h,1]);
    subject to Probi2{h in H: besti[h]=2}:
        probi[h]=gsl_ran_logistic_P(log(ic[h])-log(iparams[h,3]),1/iparams[h,4]);
    subject to Probi3{h in H: besti[h]=3}:
        probi[h]=gsl_ran_logistic_P(ic[h]-iparams[h,5],iparams[h,6]);
    subject to Probi4{h in H: besti[h]=4}:
        probi[h]=gsl_cdf_lognormal_P(ic[h],iparams[h,7],iparams[h,8]); 
    subject to Probi5{h in H: besti[h]=5}:
        probi[h]=gsl_cdf_gaussian_P(ic[h]-iparams[h,9],iparams[h,10]);
    subject to Probi6{h in H: besti[h]=6}:
        probi[h]=gsl_ran_rayleigh_P(ic[h],iparams[h,11]);
    subject to Probi7{h in H: besti[h]=7}:
        probi[h]=gsl_cdf_weibull_P(ic[h],iparams[h,13],iparams[h,14]);


    subject to Probd1{h in H,c in C: bestd[h,c]=1}:
        probd[h,c]=gsl_cdf_exponential_P(dc[h,c],dparams[c,h,1]);
    subject to Probd2{h in H,c in C: bestd[h,c]=2}:
        probd[h,c]=gsl_ran_logistic_P(log(dc[h,c])-log(dparams[c,h,3]),1/dparams[c,h,4]);
    subject to Probd3{h in H,c in C: bestd[h,c]=3}:
        probd[h,c]=gsl_ran_logistic_P(dc[h,c]-dparams[c,h,5],dparams[c,h,6]);
    subject to Probd4{h in H,c in C: bestd[h,c]=4}:
        probd[h,c]=gsl_cdf_lognormal_P(dc[h,c],dparams[c,h,7],dparams[c,h,8]);
    subject to Probd5{h in H,c in C: bestd[h,c]=5}:
        probd[h,c]=gsl_cdf_gaussian_P(dc[h,c]-dparams[c,h,9],dparams[c,h,10]);
    subject to Probd6{h in H,c in C: bestd[h,c]=6}:
        probd[h,c]=gsl_ran_rayleigh_P(dc[h,c],dparams[c,h,11]);
    subject to Probd7{h in H,c in C: bestd[h,c]=7}:
        probd[h,c]=gsl_cdf_weibull_P(dc[h,c],dparams[c,h,13],dparams[c,h,14]);
