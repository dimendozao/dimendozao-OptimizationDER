...
% Grey Wolf Optimizer
function [Alpha_score,Alpha_pos,Convergence_curve]=GWO(SearchAgents_no,Max_iter,lb,ub,dim,npv,fobj)
...
Positions=initialization(SearchAgents_no,dim,ub,lb,npv);
...
while l<Max_iter
    ...
    for n=1:size(Positions,1)
        par(n)=parfeval(fobj,1,Positions(n,:));
    end
    wait(par)
    for n=1:size(Positions,1)
        fitness(n)=fetchOutputs(par(n));
    end
    for i=1:size(Positions,1)        
