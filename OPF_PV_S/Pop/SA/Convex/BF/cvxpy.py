    ...
    for h in range(H):
        ...
        for c in range(ncluster):        
            if bestfitsd[h][c]==0:                    
                ...
                if dparams[c][h][13]>=1:
                    probd[h][c]=-cvx.power(dparams[c][h][12]*dc[h][c],dparams[c][h][13])
                else:
                    probd[h][c]=-cvx.exp(-cvx.power(dparams[c][h][12]*dc[h][c],dparams[c][h][13]))
        
        if bestfitsi[h]!=10:
            if  bestfitsi[h]==0:               
                probi[h]=cvx.exp(iparams[h][0]*ic[h])-1  #expon convex (min p)            
            elif  bestfitsi[h]==2:
                probi[h][c]=((ic[h]-iparams[h][4])/iparams[h][5])+cvx.logistic(((-ic[h]+iparams[h][4])/iparams[h][5])) # logistic concave (max 1-p) (min p)   
            elif  bestfitsi[h]==5:
