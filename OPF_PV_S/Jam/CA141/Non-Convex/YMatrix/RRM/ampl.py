...
ampl.eval(
    ...
    subject to ic0{h in H: ihours[h]=0}:
        ic[h]=0;
    ...
    subject to Probi4{h in H: besti[h]=4}:
        probi[h]=0.5+(1/(pi^0.5))*sum{k in NN} (normnum[k]*(((log(ic[h])-iparams[h,7])/iparams[h,8])^((2*(k-1)+1))))/normden[k]; 
    subject to Probi5{h in H: besti[h]=5}:
        probi[h]=0.5+(1/(pi^0.5))*sum{k in NN} (normnum[k]*(((ic[h]-iparams[h,9])/iparams[h,10])^((2*(k-1))+1)))/normden[k];
    ...
    subject to Probd4{h in H,c in C: bestd[h,c]=4}:
        probd[h,c]=0.5+(1/(pi^0.5))*sum{k in NN} (normnum[k]*(((log(dc[h,c])-dparams[c,h,7])/dparams[c,h,8])^((2*(k-1))+1)))/normden[k];
    subject to Probd5{h in H,c in C: bestd[h,c]=5}:
        probd[h,c]=0.5+(1/(pi^0.5))*sum{k in NN} (normnum[k]*(((dc[h,c]-dparams[c,h,9])/dparams[c,h,10])^((2*(k-1))+1)))/normden[k]; 
