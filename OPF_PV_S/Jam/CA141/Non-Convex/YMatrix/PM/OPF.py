...
ampl.eval(
    ...
    subject to PB {h in H,i in N}: 
       pg[h,i]+(ppv[i]*ic[h]*pveff)-sum{c in C}(cpdem[i,c]*dc[h,c]) = sum {j in N: idx1[i,j]=1} (v[h,i]*v[h,j]*Yr[i,j]*cos(ph[h,i]-ph[h,j]))+sum {j in N: idx1[i,j]=1} (v[h,i]*v[h,j]*Yi[i,j]*sin(ph[h,i]-ph[h,j]));
