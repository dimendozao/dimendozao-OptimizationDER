# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 11:37:36 2024
...
ampl.get_parameter("ic").set_values(imeans)
ampl.get_parameter("dc").set_values(dmeans)
...
ampl.eval(
    ...
    var ppb{h in H,i in N} >=-pbcmax, <=pbcmax;
    ...
    subject to PB {h in H,i in N}: 
       pg[h,i]+(ppv[i]*ic[h]*pveff)+ppb[h,i]-sum{c in C}(cpdem[i,c]*dc[h,c]) = sum {j in N:idx1[i,j]==1} (v[h,i]*v[h,j]*Yr[i,j]*cos(ph[h,i]-ph[h,j]))+sum {j in N:idx1[i,j]==1} (v[h,i]*v[h,j]*Yi[i,j]*sin(ph[h,i]-ph[h,j]));
    ...
    subject to PB1 {h in H, i in N}:
        ppb[h,i]<=zb[h,i]*pbcmax;
    subject to PB2 {h in H, i in N}:
        ppb[h,i]>=zb[h,i]*-pbcmax;
    subject to PB3 {h in H, i in N}:
        ppb[h,i]<=pb[h]+pbcmax*(1-zb[h,i]);
    subject to PB4 {h in H, i in N}:
        ppb[h,i]>=pb[h]-pbcmax*(1-zb[h,i]);

    subject to PB5 {h in H}:
        pb[h]>=-pbc;
    subject to PB6 {h in H}:
        pb[h]<=pbc;
    subject to PB7 {h in H}:
        sum{i in N} zb[h,i]==1;


    subject to CAP0{h in H: h==1}:
        cap[h]==cap0-pb[h];
    subject to CAP024{h in H: h==nh}:
        cap[h]==cap0;
    subject to CAPH{h in H: h<nh}:
        cap[h+1]==cap[h]-pb[h+1];
    subject to CAPUP{h in H}:
        cap[h]<=pbc;
    subject to CAP0UP:
        cap0<=pbc;
...
ampl.option["scip_options"] = "return_gap=1 time=21600"

#ampl.option["solver"] = "bonmin"
#ampl.option["bonmin_options"] = "bonmin.time_limit 21600"
...
pbo= np.zeros(H)
capo= np.zeros(H)
ppbo= np.zeros([H,num_nodes])
zbo= np.zeros([H,num_nodes])

pbco=ampl.get_variable('pbc').value()
cap0o=ampl.get_variable('cap0').value()


for h in range(H):
    pbo[h]=ampl.get_variable('pb')[h+1].value()
    capo[h]=ampl.get_variable('cap')[h+1].value()    
    for i in range (num_nodes):
        ...
        ppbo[h][i]=ampl.get_variable('ppb')[h+1,i+1].value()
