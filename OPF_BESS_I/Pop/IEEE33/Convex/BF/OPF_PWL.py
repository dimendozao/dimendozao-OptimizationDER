# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 09:37:59 2024
...
dsocxg=np.linspace(-1,1,npwl)
pbcxg=np.linspace(0,pbcmax,npwl)

dsocx=np.zeros([npwl,npwl])
pbcx=np.zeros([npwl,npwl])


lambdasmax=np.ones([H,npwl,npwl])
lambdasmin=np.zeros([H,npwl,npwl])
etasmax=np.ones([H,npwl])
etasmin=np.zeros([H,npwl])

xismax=np.ones([H,npwl])
xismin=np.zeros([H,npwl])

for i in range(npwl):
    dsocx[i]=dsocxg
    pbcx[:,i]=pbcxg
    
pbx=np.multiply(dsocx,pbcx)        
...
pbc= cvx.Variable(H,name='pbc')
pb=  cvx.Variable(H,name='pb')
ppb= cvx.Variable((H,num_nodes),name='ppb')
zb= cvx.Variable((H,num_nodes),boolean=True,name='zb')
soc0= cvx.Variable(name='soc0')
soc= cvx.Variable(H,name='soc')
dsoc= cvx.Variable(H,name='dsoc')

deltax= cvx.Variable((H,npwl-1),boolean=True)
deltae= cvx.Variable((H,npwl-1),boolean=True)
xis=    cvx.Variable((H,npwl),nonneg=True)
etas=   cvx.Variable((H,npwl),nonneg=True)

lambdas=[0]*H 


for h in range(H):
    lambdas[h]=  cvx.Variable([npwl,npwl],nonneg=True)
...
res += [dsoc[0]==soc0-soc[0]]
res += [soc[H-1]==soc0]


for h in range(H):    
    res += [cvx.sum(zb[h])==1]
    res += [lambdas[h]<=lambdasmax[h]]
    res += [lambdas[h]>=lambdasmin[h]]
    res += [etas[h]<=etasmax[h]]
    res += [etas[h]>=etasmin[h]]
    res += [xis[h]<=xismax[h]]
    res += [xis[h]>=xismin[h]]
    res += [pbc[h]<=pbcmax]
    res += [pbc[h]>=0]
...
for h in range(H):
    ...
    res += [pb[h]==cvx.sum(cvx.multiply(lambdas[h],pbx))]
    res += [dsoc[h]==cvx.sum(cvx.multiply(lambdas[h],dsocx))]
    res += [pbc[h]==cvx.sum(cvx.multiply(lambdas[h],pbcx))]
    
    res += [cvx.sum(lambdas[h])==1]
    res += [etas[h]==cvx.sum(lambdas[h],axis=0)]
    res += [xis[h]==cvx.sum(lambdas[h],axis=1)]
    res += [cvx.sum(deltax[h])==1]
    res += [cvx.sum(deltae[h])==1]
    for k in range(npwl):
        if k==0:
            res += [xis[h][k]-deltax[h][k]<=0]
            res += [etas[h][k]-deltae[h][k]<=0]
        elif k==npwl-1:
            res += [xis[h][k]-deltax[h][k-1]<=0]
            res += [etas[h][k]-deltae[h][k-1]<=0]
        else:
            res += [xis[h][k]-deltax[h][k-1]-deltax[h][k]<=0]
            res += [etas[h][k]-deltae[h][k-1]-deltae[h][k]<=0]
...
out1=np.vstack((ploss,qloss,pgo[:,iref],qgo[:,iref],t,imeans,pbcout,pbo,soc0out,soco)).T
