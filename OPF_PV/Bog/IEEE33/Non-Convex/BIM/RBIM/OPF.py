...
ampl.eval(
    ...
    minimize Losses:
        sum{i in N,k in L: fr[k]=i} yr[k]*(vr[i]*vr[i]+vi[i]*vi[i])
       -sum{i in N,k in L: fr[k]=i} yr[k]*(vr[i]*vr[to[k]]+vi[i]*vi[to[k]])
       +sum{i in N,k in L: fr[k]=i} yi[k]*(vr[i]*vi[to[k]]-vi[i]*vr[to[k]])
       +sum{i in N,k in L: to[k]=i} yr[k]*(vr[i]*vr[i]+vi[i]*vi[i])
       -sum{i in N,k in L: to[k]=i} yr[k]*(vr[i]*vr[fr[k]]+vi[i]*vi[fr[k]])
       +sum{i in N,k in L: to[k]=i} yi[k]*(vr[i]*vi[fr[k]]-vi[i]*vr[fr[k]])
       -sum{i in N,k in L: fr[k]=i} yi[k]*(vr[i]*vr[i]+vi[i]*vi[i])
       +sum{i in N,k in L: fr[k]=i} yi[k]*(vr[i]*vr[to[k]]+vi[i]*vi[to[k]])
       +sum{i in N,k in L: fr[k]=i} yr[k]*(vr[i]*vi[to[k]]-vi[i]*vr[to[k]])
       -sum{i in N,k in L: to[k]=i} yi[k]*(vr[i]*vr[i]+vi[i]*vi[i])
       +sum{i in N,k in L: to[k]=i} yi[k]*(vr[i]*vr[fr[k]]+vi[i]*vi[fr[k]])
       +sum{i in N,k in L: to[k]=i} yr[k]*(vr[i]*vi[fr[k]]-vi[i]*vr[fr[k]]);
    
    subject to PB {i in N}: 
        pg[i]+(ppv[i]*rad*pveff)-pd[i] = sum {k in L: fr[k]=i} yr[k]*(vr[i]*vr[i]+vi[i]*vi[i])
