...
for i=1:NT
    ...
    popini=initialization(NS,nvar,ub,lb);
    t1 = tic;
    opts = optimoptions('particleswarm', 'SwarmSize',NS, 'InitialPoints',popini, 'MaxIterations',NI,'MaxStallIterations',NI,'UseParallel',true);
    [Best_pos(i,:),Best_score(i)]=particleswarm(f,nvar,lb,ub,opts);
    tr(i) = toc(t1); 
end
