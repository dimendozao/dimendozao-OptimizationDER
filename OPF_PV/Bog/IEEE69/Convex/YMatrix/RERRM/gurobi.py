...
m = gp.Model("PF-rect")
pgref = m.addMVar(num_nodes,lb=0,ub=prefmax,name='pgref')
qgref = m.addMVar(num_nodes,lb=0,ub=qrefmax,name='qgref')
...
m.addConstrs((pgref[i]+(ppv[i]*rad)+pgen[i]-pdm[i]==EqNp[i] for i in range(num_nodes)), name='c-pf-p')
