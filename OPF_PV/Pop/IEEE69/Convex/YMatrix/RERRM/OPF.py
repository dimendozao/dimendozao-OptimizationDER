...
ampl.get_parameter("imeans").set(imeans)  
...
ampl.eval(
    ...
    subject to PB {h in H,i in N}: 
        pg[h,i]+(ppv[i]*imeans[h])-pd[h,i] = sum {j in N: idx1[i,j]=1} cn[h,i,j]*yr[i,j] + sum {j in N: idx1[i,j]=1} sn[h,i,j]*yi[i,j];  
    ...
    subject to PL{h in H}:
        pl[h]=sum {i in N, j in N: idx1[i,j]=1} cn[i,j]*yr[i,j] + sum {i in N,j in N: idx1[i,j]=1} sn[i,j]*yi[i,j];    
    subject to QL{h in H}:
        ql[h]=sum {i in N, j in N: idx1[i,j]=1} sn[i,j]*yr[i,j] - sum {i in N,j in N: idx1[i,j]=1} cn[i,j]*yi[i,j]; 
