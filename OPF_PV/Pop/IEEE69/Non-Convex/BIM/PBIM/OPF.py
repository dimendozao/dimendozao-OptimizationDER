...
ampl.eval(
    ...
    minimize Losses:
        sum{i in N, k in L: fr[k]=i} v[i]*v[i]*yr[k]
       -sum{i in N, k in L: fr[k]=i} v[i]*v[to[k]]*yr[k]*cos(ph[i]-ph[to[k]])
       -sum{i in N, k in L: fr[k]=i} v[i]*v[to[k]]*yi[k]*sin(ph[i]-ph[to[k]])
       +sum{i in N, k in L: to[k]=i} v[i]*v[i]*yr[k]
       -sum{i in N, k in L: to[k]=i} v[i]*v[fr[k]]*yr[k]*cos(ph[i]-ph[fr[k]])
       -sum{i in N, k in L: to[k]=i} v[i]*v[fr[k]]*yi[k]*sin(ph[i]-ph[fr[k]])
       -sum{i in N, k in L: fr[k]=i} v[i]*v[i]*yi[k]
       +sum{i in N, k in L: fr[k]=i} v[i]*v[to[k]]*yi[k]*cos(ph[i]-ph[to[k]])
       -sum{i in N, k in L: fr[k]=i} v[i]*v[to[k]]*yr[k]*sin(ph[i]-ph[to[k]])
       -sum{i in N, k in L: to[k]=i} v[i]*v[i]*yi[k]
       +sum{i in N, k in L: to[k]=i} v[i]*v[fr[k]]*yi[k]*cos(ph[i]-ph[fr[k]])
       -sum{i in N, k in L: to[k]=i} v[i]*v[fr[k]]*yr[k]*sin(ph[i]-ph[fr[k]]);
       
    
    subject to PB {i in N}: 
        pg[i]+(ppv[i]*rad*pveff)-pd[i] = sum {k in L: fr[k]=i} v[i]*v[i]*yr[k]
