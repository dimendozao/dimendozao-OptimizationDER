...
ampl.eval(
    ...
    minimize Losses:
       sum{i in N, j in N} vr[i]*vr[j]*ymr[i,j] 
       - sum {i in N, j in N} vr[i]*vi[j]*ymi[i,j]
       + sum{i in N, j in N} vi[i]*vi[j]*ymr[i,j] 
       + sum{i in N, j in N} vi[i]*vr[j]*ymi[i,j]
       -sum {i in N, j in N} vr[i]*vi[j]*ymr[i,j]
       - sum {i in N, j in N} vr[i]*vr[j]*ymi[i,j]
       + sum{i in N, j in N} vi[i]*vr[j]*ymr[i,j] 
       - sum{i in N, j in N} vi[i]*vi[j]*ymi[i,j];
    
    subject to PB {i in N}: 
        pg[i]+(ppv[i]*rad*pveff)-pd[i] = sum{j in N} vr[i]*vr[j]*ymr[i,j] 
        - sum {j in N} vr[i]*vi[j]*ymi[i,j]
        + sum{j in N} vi[i]*vi[j]*ymr[i,j] 
        + sum{j in N} vi[i]*vr[j]*ymi[i,j];  
        
    subject to QB {i in N}: 
        qg[i]-qd[i] = -sum {j in N} vr[i]*vi[j]*ymr[i,j]
        - sum {j in N} vr[i]*vr[j]*ymi[i,j]
        + sum{j in N} vi[i]*vr[j]*ymr[i,j] 
        - sum{j in N} vi[i]*vi[j]*ymi[i,j];
...
for i in range(num_nodes):
    for j in range(num_nodes):
        Equp[i]+=(vro[i]*vro[j]*ymr[i][j])-(vro[i]*vio[j]*ymi[i][j])+(vio[i]*vio[j]*ymr[i][j])+(vio[i]*vro[j]*ymi[i][j])
        Equq[i]+=-(vro[i]*vio[j]*ymr[i][j])-(vro[i]*vro[j]*ymi[i][j])+(vio[i]*vro[j]*ymr[i][j])-(vio[i]*vio[j]*ymi[i][j])
