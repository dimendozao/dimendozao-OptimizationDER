...
for i in range(nform):
    if i<nform-4 and not i==6 and not i==11:
        ...
        datadict.update({form[i]:pd.read_excel('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city+'\\'+case+'\\'+conv[i]+'\\'+fold[i]+'\\Results.xlsx')})
        metadict.update({form[i]:pd.read_excel('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city+'\\'+case+'\\'+conv[i]+'\\'+fold[i]+'\\MetaResults.xlsx')})
...
for i in range(nform):
    infeas1[i]=np.sum(np.abs(datadict[form[i]]['pg'].to_numpy()+(datadict[form[i]]['ppv'].to_numpy()*rad)-datadict[form[i]]['eqp'].to_numpy()-pdm))+np.sum(np.abs(datadict[form[i]]['qg'].to_numpy()-datadict[form[i]]['eqq'].to_numpy()-qdm))
    ...
    infeas2[i]=np.sum(np.abs(datadict[form[i]]['pg'].to_numpy()+(datadict[form[i]]['ppv'].to_numpy()*rad)-np.real(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-pdm))+np.sum(np.abs(datadict[form[i]]['qg'].to_numpy()-np.imag(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-qdm))
...
for i in range(nform):
    for j in range(nn):
        if datadict[form[i]]['zpv'][j]==1 and datadict[form[i]]['ppv'][j]>1e-3:
            solutions[i]+='$('+str(j+1)+','+np.format_float_scientific(datadict[form[i]]['ppv'][j], precision=2)+')'            
        


solutionsdf=pd.DataFrame(solutions,index=form,columns=['Allocation Solution $(z^{pv},PPV)$'])
...
for i in range(nmetas):
    for j in range(nmov):
        ...
        if j==1:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.argsort(metadict[metas[i]]['obj'])[0], precision=2)+'$'
        if j==2:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.mean(metadict[metas[i]]['obj']), precision=2)+'$'
        if j==3:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.std(metadict[metas[i]]['obj']), precision=2)+'$'
        if j==4:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.argsort(metadict[metas[i]]['t'])[0], precision=2)+'$'
        if j==5:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.mean(metadict[metas[i]]['t']), precision=2)+'$'
        if j==6:
            metaoverviewdf[metaov[j]][metas[i]]='$'+np.format_float_scientific(np.std(metadict[metas[i]]['t']), precision=2)+'$'
...
latexsolutions = solutionsdf.to_latex(index=True, escape=False, column_format='|c| c|') 
latexmetas = metaoverviewdf.to_latex(index=True, escape=False, column_format='|c| c c c c c c c|')

file = open('Solutions_tab.txt','w')
file.write(latexresults)
file.close() 

file = open('Metas_tab.txt','w')
file.write(latexoverview)
file.close()                 
