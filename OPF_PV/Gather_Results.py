...
for ct in range(nct):
    for ca in range(nca):
        ...
        for k in range(num_lines):
            sd[branch['j'][k]-1]=(branch['pj'][k]+1j*branch['qj'][k])*dem[cnode[branch['j'][k]-1]]
        ...
        for i in range(nform):
            if i<nform-4 and not i==6 and not i==11:
                datadict.update({form[i]:pd.read_excel('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city[ct]+'\\'+case[ca]+'\\'+conv[i]+'\\'+fold[i]+'\\'+form[i]+'\\Results.xlsx',index_col=0)})
            elif i==6 or i==11:
                datadict.update({form[i]:pd.read_excel('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city[ct]+'\\'+case[ca]+'\\'+conv[i]+'\\'+fold[i]+'\\Results.xlsx',index_col=0)})
            else:
                datadict.update({form[i]:pd.read_excel('C:\\Users\\diego\\OneDrive\\Desktop\\aLL\\PhD\\Tesis\\Python\\'+problem+'\\'+city[ct]+'\\'+case[ca]+'\\'+conv[i]+'\\'+fold[i]+'\\Results.xlsx')})
        ...
        vs=datadict[form[nform-1]].columns.tolist()
        vs.remove('eqp')
        vs.remove('eqq')
        vs.remove('pv')
        vs.remove('ppv')
        ...
        for j in range(nform):
            ...
            losses[j]=datadict[form[j]]['pl'][0]
        
        for i in range(nform):
            infeas1[i]=np.sum(np.abs(datadict[form[i]]['pg'].to_numpy()+(datadict[form[i]]['ppv'].to_numpy()*rad*pveff)-datadict[form[i]]['eqp'].to_numpy()-pdm))+np.sum(np.abs(datadict[form[i]]['qg'].to_numpy()-datadict[form[i]]['eqq'].to_numpy()-qdm))
            vmat=np.multiply(datadict[form[i]]['v'].to_numpy(),np.cos(np.abs(datadict[form[i]]['ph'].to_numpy())))+1j*np.multiply(datadict[form[i]]['v'].to_numpy(),np.sin(datadict[form[i]]['ph'].to_numpy()))
            infeas2[i]=np.sum(np.abs(datadict[form[i]]['pg'].to_numpy()+(datadict[form[i]]['ppv'].to_numpy()*rad*pveff)-np.real(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-pdm))+np.sum(np.abs(datadict[form[i]]['qg'].to_numpy()-np.imag(np.multiply(vmat,np.matmul(ym.conjugate(),vmat.conjugate())))-qdm))
        ...
        for i in range(nform):
            for j in range(ndiffs):
                results[i][j]=np.sum(np.square(datadict[form[i]][vs[j]].to_numpy()-datadict[form[idxd[0]]][vs[j]].to_numpy()))
        ...
        solutions=[['']*3 for i in range(nform)]
        
        
        
        for i in range(nform):
            k=0
            solutions[i][0]='$'+np.format_float_scientific(datadict[form[i]]['pl'][0], precision=2)+')$'
            for j in range(nn):
                if datadict[form[i]]['zpv'][j]==1 and datadict[form[i]]['ppv'][j]>1e-3:
                    solutions[i][1]+='$('+str(j+1)+','+np.format_float_scientific(datadict[form[i]]['ppv'][j], precision=2)+')$'
                    k+=1
            solutions[i][2]+=str(k) 
        
        
        npv=np.sum(datadict[form[idxd[0]]]['zpv'].to_numpy())
        nvarssol=npv+4
        varssol=[['']*nvarssol]
        ...
        sumcap=0
        sumnpv=0
        bestsolutions[0][k]='$'+np.format_float_scientific(datadict[form[idxd[0]]]['pl'][0], precision=4)+'$'
        k+=1
        for j in range(nn):
            if datadict[form[idxd[0]]]['zpv'][j]==1:
                bestsolutions[0][k]+='$('+str(j+1)+','+np.format_float_scientific(datadict[form[idxd[0]]]['ppv'][j], precision=4)+')$'
                varssol[0][k]+='$(Loc'+str(k)+',Cap'+str(k)+')$'
                sumcap+=datadict[form[idxd[0]]]['ppv'][j]
                sumnpv+=1
                k+=1
        ...
        k+=1
        varssol[0][k]='$V_{max}$'
        bestsolutions[0][k]+='$'+np.format_float_scientific(np.max(datadict[form[idxd[0]]]['v'].to_numpy()), precision=4)+'$'
        k+=1
        varssol[0][k]='$V_{min}$'
        bestsolutions[0][k]+='$'+np.format_float_scientific(np.min(datadict[form[idxd[0]]]['v'].to_numpy()), precision=4)+'$'
        
        solutionsdf=pd.DataFrame(solutions,index=form,columns=['$P_{loss}','Allocation Solution $(z^{pv},PPV)$','N_{pv}'])
