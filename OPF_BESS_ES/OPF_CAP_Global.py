...
for ct in range(nct):
    for ca in range(nca):
        if ct>0 or ca>=3:
            ...
            pbre= m.addMVar(H,lb=0.5,ub=2,name='pbre')
            pbe= m.addMVar(H,lb=-10,ub=10,name='pbe')
            ...
            m.addConstrs((pbe[h]==pb[h]*pbre[h] for h in range(H)),name='c-pbre')
            ...
            if m.SolCount>0:
                ...
                out1=np.vstack((ploss,qloss,pgo[:,iref],qgo[:,iref],t,gap,ic,pbcout,pbo,pbeo,pbreo,cap0out,capo)).T
...
#gapsdf.to_csv('CAP_GAPS.csv',header=False, index=False)