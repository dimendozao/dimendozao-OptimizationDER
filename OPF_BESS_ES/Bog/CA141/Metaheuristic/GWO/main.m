...
for h=1:H
    if h==1
        ...
        if pbr(h)<=0
            pbre(h)=1/(1.5-(0.5*sqrt(1-pbr(h))));
        else
            pbre(h)=0.5+(0.5*sqrt(1-pbr(h)));
        end
        pbe(h)=pbre(h)*pb(h);
    elseif h<H
        pbr(h)=(soc(h-1)-soc(h));
...
tab=table(v,ph,eqp,eqq,plh,qlh,pg,qg,t,gap,ico,pbco,pb,pbe,pbre,soc0,soc,zb,dco,ppvo);
